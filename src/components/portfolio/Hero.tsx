...
import { ArrowRight, Mail, Code2, Database, Cpu, Rocket, Github, Sparkles } from "lucide-react";

const roles = ["Software Engineer", "Full Stack Developer", "Python Developer"];
    ...
    <section id="home" className="relative min-h-screen flex items-center pt-28 pb-20 overflow-hidden">
      ...
      <div className="relative mx-auto max-w-6xl px-4 sm:px-6 grid lg:grid-cols-[1.2fr_1fr] gap-12 items-center">
        <div>
          ...
          >
            Passionate software engineer focused on building modern, scalable, and user-friendly
        ...
        >
          <div className="relative h-72 w-72 sm:h-80 sm:w-80">
            ...
            <div className="absolute inset-6 rounded-full border border-[var(--neon-purple)]/30" style={{ animation: "spin 24s linear infinite reverse" }} />
            <div className="absolute inset-12 rounded-full border border-[var(--neon-cyan)]/20" />
            {/* Orbiting dots */}
            <div className="absolute inset-0 grid place-items-center">
              <div className="relative h-full w-full">
                <span className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" style={{ animation: "orbit 14s linear infinite" }}>
                  <span className="block h-3 w-3 rounded-full bg-[var(--neon-cyan)] shadow-[0_0_18px_var(--neon-cyan)]" />
                </span>
              </div>
            </div>
            {/* Profile circle */}
            <div className="absolute inset-10 rounded-full p-[2px] bg-gradient-to-br from-[var(--neon-cyan)] via-[var(--neon-blue)] to-[var(--neon-purple)] animate-glow-pulse">
              <div className="h-full w-full rounded-full bg-[oklch(0.16_0.05_270)] grid place-items-center overflow-hidden">
                <div className="text-7xl font-bold text-gradient select-none">K</div>
              </div>
            </div>
          </div>
        </motion.div>
